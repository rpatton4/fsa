...
func Test_PostalAddressValidation(t *testing.T) {
	...
	validAddress, err := fsamodels.NewPostalAddress("123 Main St", "Apt 4B", "Under the Stairs", "Springfield", "IL", "62701", "US")
	if err != nil {
	...
	_, err = fsamodels.NewPostalAddress("1234567890123456789012345678901234567890 Main St", "", "", "Springfield", "IL", "12345678901", "US")
	...
	_, err = fsamodels.NewPostalAddress("abc", "1234567890123456789012345678901234567890 Main St", "", "Springfield", "IL", "12345678901", "US")
	if err == nil || len(err.UpstreamErrors) == 0 {
		t.Error("Expected error for invalid Address Line 2 length, but got none")
	}

	// Test invalid address line 3
	_, err = fsamodels.NewPostalAddress("abc", "def", "1234567890123456789012345678901234567890 Main St", "Springfield", "IL", "12345678901", "US")
	if err == nil || len(err.UpstreamErrors) == 0 {
		t.Error("Expected error for invalid Address Line 3 length, but got none")
	}

	// Test for invalid city
	_, err = fsamodels.NewPostalAddress("123 Main St", "", "", "1234567890123456789012345678901234567890", "IL", "12345678901", "US")
	if err == nil || len(err.UpstreamErrors) == 0 {
		t.Error("Expected error for invalid city length, but got none")
	}

	// Test for invalid domestic state code
	_, err = fsamodels.NewPostalAddress("123 Main St", "", "", "Springfield", "I", "12345678901", "US")
	if err == nil || len(err.UpstreamErrors) == 0 {
		t.Error("Expected error for invalid domestic state length, but got none")
	}

	// Test for invalid international state code
	_, err = fsamodels.NewPostalAddress("123 Main St", "", "", "Springfield", "1234567890123456789012345678901234567890", "12345678901", "UK")
	if err == nil || len(err.UpstreamErrors) == 0 {
		t.Error("Expected error for invalid international state length, but got none")
	}

	// Test invalid postal code
	_, err = fsamodels.NewPostalAddress("123 Main St", "", "", "Springfield", "IL", "12345678901234567890123456789012345678901234567890123456789012345678901234567890", "US")
	if err == nil || len(err.UpstreamErrors) == 0 {
