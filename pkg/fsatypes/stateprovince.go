...
func NewDomesticStateProvince(s string) (StateProvince, *fsaservices.FSAError) {
	if len(s) != exactDomesticStateCodeLength {
		return "", &fsaservices.FSAError{
			Code:    fsaservices.PostalAddressStateProvinceLengthInvalid,
...
func NewInternationalStateProvince(s string) (StateProvince, *fsaservices.FSAError) {
	l := len(s)
